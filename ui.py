...
class QuizInterface:
    def __init__(self, quizBrain: QuizBrain) -> None:
        ...
        self.trueButton = Button(image=trueImg, highlightthickness=0, command=self.truePressed)
        self.trueButton.grid(row=2, column=0)

        falseImg = PhotoImage(file="images/false.png")
        self.falseButton = Button(image=falseImg, highlightthickness=0, command=self.falsePressed)
    ...
    def getNextQuestion(self):
        ...
        if self.quiz.still_has_questions():
            ...
            self.canvas.itemconfig(self.questionText, text="You've reached the end od the quiz")
